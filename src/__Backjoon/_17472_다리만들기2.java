...
public class _17472_다리만들기2 {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < M; j++) {
				if(map[i][j] != 0 && start == 0)
					...
					if(len >= 2)
						cons[start][end] = cons[end][start] = Math.min(len, cons[start][end] == 0? Integer.MAX_VALUE: cons[start][end]);
					start = end;
					len = 0;
					endready = false;
				}
		...
		while(!Q.isEmpty()) {
			...
			for (int i = 1; i <= cnt; i++) {
				if(visited[i] || cons[current.vertex][i] == 0) continue;
				Q.add(new Vertex(i, cons[current.vertex][i]));
			}
		}
